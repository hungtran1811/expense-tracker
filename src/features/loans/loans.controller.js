...
function getLoanInterestRate(transaction = {}) {
  const rate = Number(transaction?.interestRate || 0);
  return Number.isFinite(rate) && rate > 0 ? rate : 0;
}

function getLoanInterestAmount(transaction = {}) {
  if (String(transaction?.type || "").trim() !== "loan_lend") return 0;
...
function getLoanReceivableAmount(transaction = {}) {
  const principal = Math.abs(Number(transaction?.amount || 0));
  if (String(transaction?.type || "").trim() !== "loan_lend") return principal;
  return principal + getLoanInterestAmount(transaction);
}

function formatInterestRate(value = 0) {
  const rate = Math.max(0, Number(value || 0));
  return `${new Intl.NumberFormat("vi-VN", { maximumFractionDigits: 2 }).format(rate)}%`;
}
...
function groupLoanTimeline(items = [], accountMap) {
  ...
  items.forEach((transaction) => {
    ...
    const interestRate = getLoanInterestRate(transaction);
    const receivableAmount = getLoanReceivableAmount(transaction);

    groups.get(dateLabel).items.push({
      id: String(transaction?.id || "").trim(),
      type,
      typeLabel: getTransactionTypeLabel(transaction?.type),
      amountText: type === "loan_lend" ? `-${formatCurrency(amount)}` : `+${formatCurrency(amount)}`,
      amountClass: type === "loan_lend" ? "expense" : "income",
      accountLabel: String(account?.name || "Không rõ").trim(),
      note: String(transaction?.note || "").trim(),
      metaItems:
        type === "loan_lend" && interestRate > 0
          ? [`Lãi ${formatInterestRate(interestRate)}`, `Phải thu ${formatCurrency(receivableAmount)}`]
          : [],
    });
  });
...
function getLoanOutstandingAmount(partyId = "", transactions = [], excludeTransactionId = "") {
  ...
  return (Array.isArray(transactions) ? transactions : []).reduce((sum, transaction) => {
    ...
    if (String(transaction?.type || "").trim() === "loan_lend") return sum + getLoanReceivableAmount(transaction);
    const amount = Math.abs(Number(transaction?.amount || 0));
    return sum - amount;
  }, 0);
}
...
export function sanitizeLoanEntryDraft(payload = {}) {
  ...
  return {
    ...
    accountId,
    amount,
    interestRate: type === "loan_lend" ? interestRate : 0,
    occurredAt,
    note,
  };
}
...
export function buildLoanEntryContext({ draft = {}, parties = [], transactions = [] } = {}) {
  ...
  const interestRate = type === "loan_lend" ? Math.max(0, Number(draft?.interestRate || 0)) : 0;
  const interestAmount = type === "loan_lend" ? (amount * interestRate) / 100 : 0;
  const receivableAmount = type === "loan_lend" ? amount + interestAmount : amount;
  const outstandingAfter =
    type === "loan_repay" ? outstandingBefore - amount : outstandingBefore + receivableAmount;
  ...
  const contextNote =
    type === "loan_repay"
      ...
      : interestRate > 0
        ? `Khoản này có lãi ${formatInterestRate(interestRate)}, tổng phải thu ${formatCurrency(receivableAmount)}.`
  ...
  return {
    ...
    principalText: formatCurrency(amount),
    interestRate,
    interestRateText: formatInterestRate(interestRate),
    interestAmount,
    interestAmountText: formatCurrency(interestAmount),
    receivableAmount,
    receivableAmountText: formatCurrency(receivableAmount),
    isOverpay,
    note: contextNote,
  };
}
...
export function buildLoansVm({
  ...
  const partyItems = orderedParties
    .map((party) => {
      ...
      return {
        ...
        metaItems: [
          `Đã mượn ${formatCurrency(stats.lendTotal || 0)}`,
  ...
  const totals = loanTransactions.reduce(
    (acc, transaction) => {
      ...
      if (String(transaction?.type || "").trim() === "loan_lend") {
        acc.lent += amount;
        acc.interest += getLoanInterestAmount(transaction);
      }
      if (String(transaction?.type || "").trim() === "loan_repay") acc.repaid += amount;
      return acc;
    },
    { lent: 0, interest: 0, repaid: 0 }
  );
  ...
  return {
    summary: {
      ...
      interestTotalText: formatCurrency(totals.interest),
